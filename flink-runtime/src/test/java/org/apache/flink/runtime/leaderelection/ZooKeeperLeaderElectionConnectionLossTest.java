...
public class ZooKeeperLeaderElectionConnectionLossTest extends TestLogger {
    ...
    public void testKeepLeadershipOnConnectionLoss() throws Exception {
        ...
        configuration.setString(HighAvailabilityOptions.HA_ZOOKEEPER_LATCH_PATH, LATCH_PATH);
        configuration.setString(HighAvailabilityOptions.HA_ZOOKEEPER_LEADER_PATH, LEADER_PATH);

        CuratorFramework client = ZooKeeperUtils.startCuratorFramework(configuration);
        LeaderElectionService leaderElectionService = ZooKeeperUtils.createLeaderElectionService(client, configuration);

        try {
            final OneShotLatch connectionLossLatch = new OneShotLatch();
            final OneShotLatch reconnectedLatch = new OneShotLatch();
            client.getConnectionStateListenable().addListener(new TestingConnectionStateListener(connectionLossLatch, reconnectedLatch));

            final OneShotLatch grantLeadershipLatch = new OneShotLatch();
            final OneShotLatch revokeLeadershipLatch = new OneShotLatch();
            leaderElectionService.start(new TestingContender(grantLeadershipLatch, revokeLeadershipLatch));
    ...
    private static final class TestingContender implements LeaderContender {
        ...
        public TestingContender(OneShotLatch grantLeadershipLatch, OneShotLatch revokeLeadershipLatch) {
    ...
    private static final class TestingConnectionStateListener implements ConnectionStateListener {
        ...
        public TestingConnectionStateListener(OneShotLatch connectionLossLatch, OneShotLatch reconnectedLatch) {
